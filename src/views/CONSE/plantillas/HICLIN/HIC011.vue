  ...
  <v-container>
    <v-row no-gutters>
      ...
      <v-col class="col-12 font-weight-bold text-center text-uppercase">
        formato de solicitud suspención maniobras de reanimación RCP
      </v-col>
      <v-col class="col-12 pt-5 text-justify">
        <p>
          Yo
          <span class="font-weight-bold">{{ reg_acomp.descrip }},</span>
          mayor de edad identificada(o) con el documento N°:
          ...
          consciente y como representante con el parentesco de
          <span class="font-weight-bold">
            {{ descripParentesco(datos_session.parentesco) }}
          </span>
          del paciente
        ...
        <p>
          He sido informado que la condicion del paciente y solicito al personal
          medico del
          <span class="font-weight-bold">{{ entidad.NOMUSU }}</span>
          para que suspenda las acciones de reanimacion (conjunto de medidas
          terapeuticas que se aplican para recuperar o mantener las constantes
          vitales del organismo) de mi familiar.
        </p>
        <p>
          En el caso y nivel de atencion del
          <span class="font-weight-bold">{{ "PENDIENTE" }},</span>
          las que corresponden a reanimacion cardiocerebro-pulmonar, las cuales
          estan destinadas a asegurar la oxigenacion de los organos vitales
          cuando la persona ha dejado de respirar o el corazon ha cesado de
          palpitar
        </p>
      </v-col>
      <v-col class="pt-5" cols="12">
        Por lo anterior manifiesto que conozco que todas las decisiones medicas
        se asientan en dos factores, el primero de ellos es tecnico, se refiere
        al concepto medico deacuerdo a la lexartis y el segundo es la aceptacion
        del paciente o familiar, en este caso actuando como representante del
...
export default {
  ...
  async created() {
    ...
    //     titulo: 'El paciente debe tener acompañante',
    //     texto: '',
    //     btn_salir: true,
    //   })
    // } else this.get_historia()
