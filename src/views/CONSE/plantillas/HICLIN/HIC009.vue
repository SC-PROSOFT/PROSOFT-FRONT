<template>
  <v-container>
    <v-row no-gutters>
      ...
      <v-col class="col-12 font-weight-bold text-center text-uppercase">
        Consentimiento informado para ingreso a hospitalización
      </v-col>
    </v-row>
    <v-row>
      <v-col class="col-12 text-justify">
        <p v-if="paci_firma">
          Yo
          ...
          __________________________________ declaro que:
        </p>
        <p v-else>
          Yo
          <span class="font-weight-bold">{{ reg_acomp.descrip }},</span>
          mayor de edad identificado con el documento N°:
          <span class="font-weight-bold">{{ mask_id(reg_acomp.cod) }}</span>
          de
          <span class="font-weight-bold">{{ reg_acomp.descrip_ciudad }}</span>
          actuando en nombre propio en pleno uso de mis facultades, libre y
          consciente, o como responsable del menor
          <span class="font-weight-bold">{{ reg_paci.descrip }}</span>
          declaro que:
      ...
      <v-col class="col-12 text-justify">
        <ol>
          <li>
            <p>
              Otorgo mi consentimiento para el ingreso al servicio de
              HOSPITALIZACION de acuerdo a los diagnósticos y plan de
              tratamiento, por orden del médico:
              <span class="font-weight-bold">{{ reg_hc.descrip_med }}</span>
            </p>
          </li>
          <li>
            <p>
              Acepto los servicios adicionales que requiera dentro de la
              estancia hospitalaria como canalización, estudios de laboratorios,
              radiográficos, así como la administración de medicamentos.
            </p>
          </li>
          <li>
            <p>
              Certifico que me han explicado las NORMAS DE LA INSTITUCIÓN PARA
              PACIENTES HOSPITALIZADOS, los horarios de visita y las
              restricciones.
            </p>
          </li>
          <li>
            <p>
              Acepto el ingreso al servicio de hospitalización y entiendo que
              hay riesgos como las caídas, por lo cual se deben mantener arriba
              las barandas de las camas.
            </p>
          </li>
          <li>
            <p>
              Tengo derecho a determinar a quien se puede proporcionar
              información sobre mi estado de salud, incluido la ampliación de
              esta información, y las personas a quienes no se les puede
              proporcionarla esta, soy consciente de que, en caso de
              empeoramiento de mi estado de salud, se les informara a las
              personas designadas y fue informado sobre la posibilidad de
              determinación en la toma de decisión en mi lugar si no puedo
              pronunciar el consentimiento.
            </p>
          </li>
          <li>
            <p>
              Prohíbo la prestación de información sobre mi estado de salud a
              cualquiera de las siguientes personas:
            </p>
            <v-text-field
              outlined
              label="Digite aquí el nombre de dichas personas"
              v-model="text_edit.tipo_tuberculosis"
              maxlength="90"
              dense
              @input="
                HIC009.rechazo_pers = text_edit.rechazo_pers.replaceEsp().enterReplace()
              "
            ></v-text-field>
          </li>
          <li>
            <p>
              Fui informado, que está prohibido grabar videos, audios o hacer
              grabaciones audiovisuales durante el tiempo de estancia
              hospitalaria hacia el personal médico y asistencial.
            </p>
          </li>
          <li>
            <p>
              Fui informado sobre la prohibición de fumar en el área de
              hospitalización.
            </p>
          </li>
        </ol>
...
export default {
  ...
  watch: {
    text_edit: {
      ...
      handler(val) {
        this.$nextTick(() => {
          this.text_edit.rechazo_pers = val ? val.rechazo_pers.replaceEsp() : val.rechazo_pers;
        })
      }
    }
  },
