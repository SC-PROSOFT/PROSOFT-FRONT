      ...
      <v-card-text class="text-center primary--text py-0 pb-5 mt-6">
        <v-row class="input-row">
          <v-col cols="12" sm="2" md="2" class="input-col">
            <INPUT @next-action="nextStep(form_informe, $event, datoAno)" :field="form_informe.anio" :reg="reg" />
          </v-col>
        </v-row>
      </v-card-text>
    </v-card>
...
<script>
import { getInformeTrimestral, getInformeTrimestral_ } from "../../fuentes/correspondencia/regInformeTrimestral";
  ...
  methods: {
    ...
    async consultarCorrespondencia() {
      try {
        ...
        if (RES.msg) {
          this.CON851("PNZ", "info", "No existen datos de la correspondencia de este año", null, () => {
            this.focusInput(this.form_informe, "anio");
          });
        }
      } catch (error) {
