  ...
  <v-dialog v-model="estado" persistent>
    <v-card flat>
      <v-toolbar color="primary" class="white--text">
        <h3 class="mx-auto">1-7 CREAR HOLDING EMPRESARIAL CORRESPONDENCIA.</h3>
        <v-btn icon dark @click="exitCOR885" class="botone">
...
export default {
  ...
  computed: {
    ...mapGetters({
      get: "formularios/get",
      getLista: "holding/getLista",
    }),
  },

  mounted() {
    this.consultaHolding();
    this.focus_table = true;
  },

  methods: {
    ...mapActions({
      _getHoldingF8: "holding/_getHoldingF8",
    }),
    ...
    async nextDataValidation(desde, cantidad, filtro) {
      await this._getHoldingF8({ desde, cantidad, filtro });
    ...
    async consultaHolding() {
      let f8 = this.f8;
      let desde = 0;
      let filtro = "";
      let cantidad = 9;
      const RES = await this._getHoldingF8({ desde, cantidad, filtro });
