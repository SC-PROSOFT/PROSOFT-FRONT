  ...
  <v-dialog v-model="estado" persistent>
    <v-card>
      <v-toolbar color="primary" class="white--text">
        <h3 class="mx-auto">CONSULTA DE CODIGO UNIDAD FUNCIONAL</h3>
        <v-btn icon dark @click="exitCOR876" class="botone">
...
export default {
  ...
  methods: {
    ...mapActions({
      _getUnifunF8: "unifun/_getUnifunF8",
    }),
    datoBusqueda() {
    ...
    async nextDataValidation(desde, cantidad, filtro) {
      await this._getUnifunF8({ desde, cantidad, filtro });
    ...
    async consultaUnifun() {
      ...
      let filtro = "";
      let cantidad = 9;
      const RES = await this._getUnifunF8({ desde, cantidad, filtro });
      console.log(RES);
