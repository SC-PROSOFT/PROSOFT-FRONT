  ...
  <v-dialog v-model="estado" persistent>
    <v-card flat>
      <v-toolbar color="primary" class="white--text">
        <h3 class="mx-auto">CONSULTA DE CODIGOS DE CARGOS.</h3>
        <v-btn icon dark @click="exitCOR888" class="botone">
...
export default {
  ...
  methods: {
    ...
    async nextDataValidation(desde, cantidad, filtro) {
      await this._getCargoF8({ desde, cantidad, filtro });
    ...
    async consultaCargo() {
      ...
      let filtro = "";
      let cantidad = 9;
      const RES = await this._getCargoF8({ desde, cantidad, filtro });
