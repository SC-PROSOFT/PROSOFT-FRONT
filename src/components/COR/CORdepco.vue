<template>
    <v-dialog v-model="estado" persistent width="1000">
      <v-card falt>
        <v-toolbar color="primary" class="white--text">
          <h3 class="mx-auto">VENTANA DEPCO</h3>
          <v-btn icon dark @click="exitCORdepco" class="botone">
  ...
  export default {
    ...
    methods: {
      ...
      async nextDataValidation(desde, cantidad, filtro) {
        const RES = await this._getDepcoF8({
          desde,
          cantidad,
          filtro,
        });
