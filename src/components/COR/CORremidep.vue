<template>
    <v-dialog v-model="estado" persistent width="700">
      <v-card falt>
        <v-toolbar color="primary" class="white--text">
          <h3 class="mx-auto">VENTANA REMIDEP</h3>
          <v-btn icon dark @click="exitCORremidep" class="botone">
  ...
  export default {
    ...
    computed: {
      ...mapGetters({
        get: "formularios/get",
        getlista: "remidep/getLista",
      }),
    },
    mounted() {
      this.consultaRemidep();
      this.focus_table = true;
    },
    methods: {
      ...mapActions({
        _getRemidepF8: "remidep/_remidepF8",
      }),
      datoBusqueda() {
      ...
      async nextDataValidation(desde, cantidad, filtro) {
        const RES = await this._getRemidepF8({
          desde,
          cantidad,
          filtro,
        });
